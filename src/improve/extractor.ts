...
export async function extractSkill(
  ...
  // normalizeForCache strips per-request markers like "(ref-XXXX-XXXXXX)" so
  // smoke-test floods don't write a unique cache row per probe — they collapse
  // onto a single shared row that subsequent probes actually hit.
  const normalized = normalizeForCache(input)
  if (!normalized) return { cached: false, reason: 'empty_after_normalize' }
  ...
  await db
    ...
    .bind(normalized, result.final_output, mistralConfidence, new Date().toISOString(), 0)
    .run()
